...
int connectRemoteServer(char* host_addr, int port_num)
{
    ...
    if(connect(remoteSocket, (struct sockaddr *)&server_addr), (scoket_t)sizef(server_addr)<0)
    {
...
int handle_request(int clientSocketId, ParsedRequest, *request, char* tempReq) {
    ...
    while(bytes_send > 0)
    {
        bytes_send = sned(clientSocketId, buf, bytes_send, 0);
        ...
        temp_buffer = (char*)realloc(temp_buffer, temp_bufs); 
        if(bytes_send<0)
        {
...
void *thread_fn(void *socketNew) {
    ...
    if(temp!=NULL)
    {
        ...
        len = strlen(buffer); 
        ParsedRequest *request = ParsedRequest.create(); 
...
int main(int argc, char* argv[])
{
    ...
    printf("Starting proxy server at port\n", port_number);
